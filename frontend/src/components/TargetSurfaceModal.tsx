  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    while (attempt < MAX_ATTEMPTS) {
      ...
      try {
        setStatusMessage(
          attempt === 1
            ? `Sending image + "${target}" to for Segmentation...`
            : `Retrying Segmentation (Attempt ${attempt} of ${MAX_ATTEMPTS})...`
        );
        ...
        setStatusMessage(`Segmentation started ✓ — detecting "${target}" in background...`);
        break;
      } catch (err: any) {
        ...
        if (attempt < MAX_ATTEMPTS) {
          ...
          setStatusMessage(`⚠ Segmentation failed after 3 attempts. Please re-upload your image.`);
              ...
              >
                {isLaunching ? (
                  <>
                    ...
                    <span>Starting Segmentation...</span>
                  </>
                ) : !componentName.trim() ? (
