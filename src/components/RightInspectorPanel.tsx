  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-[#3e484f]/40 bg-[#141c24] space-y-3.5 flex-shrink-0">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-0.5 rounded text-[#87929a] hover:text-[#38bdf8] transition-colors"
              title={isCompressed ? "Expand style details" : "Compress style details"}
            >
              {isCompressed ? <ChevronDown className="w-3.5 h-3.5" /> : <ChevronUp className="w-3.5 h-3.5" />}
            </button>
          </div>
        ...
        {/* Selected Material Card (Compressed vs Expanded) */}
        {isCompressed ? (
          <div
            onClick={() => onOpenSpecsModal(selectedMaterial)}
            className="p-2 rounded-lg bg-[#182028] border border-[#3e484f]/40 flex items-center justify-between cursor-pointer hover:border-[#38bdf8]/50 transition-all"
          >
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              <span className="text-xs text-[#dae3ee] font-medium truncate">
                {selectedMaterial.name}
              </span>
            </div>
            <Info className="w-3.5 h-3.5 text-[#87929a] flex-shrink-0" />
          </div>
        ) : (
          <>
            <div
      ...
      {/* Categories Accordions / Search Results (Scrollable Content Container) */}
      <div className="flex-1 overflow-y-auto min-h-0 p-4 space-y-3 overscroll-contain">
        {filteredMaterials ? (
          filteredMaterials.length === 0 ? (
            <div className="p-5 rounded-xl bg-[#141c24] border border-[#3e484f]/40 text-center flex flex-col items-center justify-center">
              <span className="material-symbols-outlined text-[#87929a] text-[32px] mb-1.5">search_off</span>
              <span className="text-xs font-bold text-[#dae3ee] mb-1">Vinyl style not found</span>
              <p className="text-[11px] text-[#87929a] leading-relaxed">
              ...
              <div key={cat.id} className="border border-[#3e484f]/40 rounded-xl overflow-hidden bg-[#141c24]">
                ...
                {isOpen && (
                  <div className="p-2.5 border-t border-[#3e484f]/20 bg-[#182028]/70 space-y-2.5">
                    ...
                    {/* Material Cards */}
                    {(() => {
                      const displayedSubMaterials = catMaterials.filter(
                        (m) => openSubCategory === 'all' || m.subCategory.toLowerCase() === openSubCategory.toLowerCase()
                      );

                      if (displayedSubMaterials.length === 0) {
                        return (
                          <div className="p-3 rounded-lg bg-[#141c24]/90 border border-[#3e484f]/30 text-center flex flex-col items-center">
                            <span className="text-xs font-semibold text-[#dae3ee] mb-0.5">Vinyl style not found</span>
